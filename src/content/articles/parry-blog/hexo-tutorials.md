---
title: "快速使用 Hexo 搭建个人博客"
description: "课程讲解了目前最常用的博客搭建框架 Hexo 的使用方法以及使用技巧,分享了大量的必备插件、图床部署以及本地编辑的技巧。"
section: "parry-blog"
slug: "hexo-tutorials"
lang: "zh-CN"
status: "published"
tags: ["视频教程","Hexo","技术文章"]
publishedAt: "2017-10-09T01:00:00.000Z"
updatedAt: "2025-07-13T13:40:31.000Z"
cover: "https://devopenclub.parryqiu.com/blog_c2a91a3b4a43d03bf967dda7c7425506.png"
author: "Parry Qiu"
---
![截图](https://devopenclub.parryqiu.com/blog_c2a91a3b4a43d03bf967dda7c7425506.png)

课程讲解了目前最常用的博客搭建框架 Hexo 的使用方法以及使用技巧，分享了大量的必备插件、图床部署以及本地编辑的技巧。

<!-- more -->

## 课程章节大纲

课程持续更新中，目前更新章节如下：

* 001 - Hexo 框架介绍
* 002 - 框架的本地安装与运行
* 003 - 框架的基本结构与打包后的文件介绍
* 004 - Hexo 博客主题的安装与推荐
* 005 - 如何自定义你的博客结构
* 006 - 如何开始写博客文章以及文章的分类与标签
* 007 - 如何让代码在 Hexo 下显示的更加美观
* 008 - 本地写作时图床的最佳解决方案
* 009 - 插件的安装与必备插件介绍
* 010 - 如何让你的博客接入评论系统
* 011 - 如何给你的博客增加搜索功能
* 012 - 多个博客统计系统的接入方案分析
* 013 - 部署博客到 GitHub 的技巧
* 014 - Hexo 其他资源的介绍与分享

课程地址: [https://devopen.club/course/hexo](https://devopen.club/course/hexo)
